...
// CORRECCIÓN: Cambiado ChevronDoubleRightIcon por ChevronRightIcon
import { PencilSquareIcon, TrashIcon, EyeIcon, XMarkIcon, ChevronLeftIcon, ChevronRightIcon } from '@heroicons/vue/24/outline';
...
const totalPages = computed(() => {
    // Se asegura que si no hay órdenes, totalPages sea al menos 1 para evitar divisiones por cero, aunque Math.ceil lo maneja.
    ...
    try {
        ...
        if (response.status === 204) {
            ...
            orders.value = orders.value.filter(order => order.id !== id);
             // Reajustar la página actual si la última página queda vacía después de eliminar
            if (currentPage.value > 1 && paginatedOrders.value.length === 0) {
                currentPage.value--;
            }
        } else if (response.status === 404) {
            alert('La orden que intentas eliminar no existe.');
        } else {
...
<template>
          ...
          <tr v-for="(order, index) in paginatedOrders" :key="order.id" :class="{'bg-gray-50': index % 2 === 0, 'bg-white': index % 2 !== 0, 'hover:bg-indigo-50': true}">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
              ...
              <button 
                ...
                title="Ver Detalles"
                class="text-gray-500 hover:text-gray-900 transition duration-150 p-1 rounded hover:bg-gray-100"
              >
              ...
              <button 
                ...
                title="Editar Orden"
                class="text-indigo-600 hover:text-indigo-800 transition duration-150 p-1 rounded hover:bg-indigo-100"
              >
              ...
              <button 
                ...
                class="text-red-600 hover:text-red-800 transition duration-150 p-1 rounded hover:bg-red-100"
              >
  ...
  <div v-if="showModal && selectedOrder" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" @click="showModal = false"></div>

    <div class="flex items-center justify-center min-h-screen p-4 relative z-50">
      
      <div class="bg-white rounded-lg shadow-xl transform transition-all max-w-lg w-full p-6">
